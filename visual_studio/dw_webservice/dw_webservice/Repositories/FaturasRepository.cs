...
namespace dw_webservice.Repositories
{
    public class FaturasRepository 
    {
       ...
       string query_faturas_abertas_cabecalho =
        ...
        string query_faturas_abertas_campos_adic =
        ...
        string query_faturas_abertas_corpo =
        ...
        public FaturasRepository(IConfiguration _configuration)
        {
        ...
        public object Consultar_fatura(out bool erro_exportar, int id_usuario_login = 0, int id = 0, int id_empresa = 0, string razao_social = "", int mes = 0, int ano = 0, string nosso_numero = "", int id_situacao = 0, string abertas = "", string exportar = "N")
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(Constantes.string_conexao))
            {
                ...
                try
                {
                    ...
                    var lista_permissoes = UsuariosRepository.Consultar_lista_perm_usuario(id_usuario_login);
                    if (!lista_permissoes.Contains("bkf") && !lista_permissoes.Contains("tin") && (exportar == Constantes.NOTA_FISCAL || exportar == Constantes.BOLETO || exportar == Constantes.PIX))
                    {
                    ...
                    if (conn.State == ConnectionState.Open)
                    {
                        ...
                        if (id != 0)
                        {
                            query_where = query_where + " f.id = :id";
                    ...
                    if (!(exportar == Constantes.NAO))
                    {
                        ...
                        if (exportar == Constantes.NOTA_FISCAL)
                        {
                            ...
                            if (linhasafetadas > 0)
                            {
                                return Utils.DapperQueryToRemessaNF(result);
                            }
                            else
                            {
                        ...
                        if (exportar == Constantes.BOLETO || exportar == Constantes.PIX)
                        {
                            ...
                            if (exportar == Constantes.BOLETO)
                            {
                                if (linhasafetadas > 0)
                                {
                                    return Utils.DapperQueryToRemessaBoleto(result);
                                }
                                else
                                {
                            ...
                            else //PIX
                            {
                                if (linhasafetadas > 0)
                                {
                                    ...
                                    pix_base = "0002010102122644" +
                                          ConfigRepository.Consultar_valor_config("pix_chave_conta") +
                                          "520400005303986" +
                                          "5413<valor>" + 
                                          "5802BR" +
                                          "5900" + ConfigRepository.Consultar_valor_config("pix_nome") +
                                          "6000" + ConfigRepository.Consultar_valor_config("pix_cidade") +
                                          "6207<identificador>" + 
                                          "6304";
        ...
        public object Consultar_faturas_abertas(int id_usuario_login)
        {
            object result = null;
        ...
        public object Incluir_fatura(int id_empresa = 0, int mes = 0, int ano = 0, string nosso_numero = "", double valor = 0, string data_vct = "", int qtd_ofertas = -1, int id_situacao = 0, int id_usuario_login = 0)
        {
            object result = null;
        ...
        public object Excluir_fatura(int id = 0, int id_usuario_login = 0)
        {
            object result = null;
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(Constantes.string_conexao))
            {
                ...
                try
                {
                    ...
                    if (conn.State == ConnectionState.Open)
                    {
                        ...
                        dyParam.Add("id", id, DbType.Int32, ParameterDirection.Input);
                        var fatura = SqlMapper.QuerySingle(conn, query, param: dyParam, commandType: CommandType.Text);
                        ...
                        linhasafetadas = SqlMapper.Execute(conn, query, dyParam, transaction, null, CommandType.Text);
        ...
        public object Efetivar_faturas_abertas(int id_usuario_login)
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(Constantes.string_conexao))
            {
                ...
                try
                {
                    ...
                    if (conn.State == ConnectionState.Open)
                    {
                        ...
                        var meses = SqlMapper.QuerySingle(conn, query, param: dyParam, commandType: CommandType.Text);
        ...
        public object Atualizar_fatura(int id = 0, int mes = 0, int ano = 0, string nosso_numero = "", double valor = 0, string data_vct = "", string data_pg = "", double multa = 0, double juros = 0, double valor_pg = 0, int qtd_ofertas = -1, int id_situacao = 0, int id_usuario_login = 0)
        {
            object result = null;
        ...
        public object Processar_retorno_boleto(IFormFile file = null, int id_usuario_login = 0)
        {
            int linha = 1;
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(Constantes.string_conexao))
            {
                ...
                try
                {
                    ...
                    HeaderRetornoBoleto headerretornoboleto = new HeaderRetornoBoleto(conteudo);
                    ...
                    List<TransacaoRetornoBoleto> boletos_pagos = new List<TransacaoRetornoBoleto>();
                    while (!reader.EndOfStream)
                    {
                        linha++;
                        conteudo = reader.ReadLine();
                        TransacaoRetornoBoleto transacaoretornoboleto = new TransacaoRetornoBoleto(headerretornoboleto.Num_banco, conteudo);
                    ...
                    foreach (TransacaoRetornoBoleto boleto in boletos_pagos)
                    {
                        ...
                        {
                            retorno.Add(Baixar_boleto(boleto, id_usuario_login));
                            linha++;
                        }
                    }
        ...
        public object Processar_retorno_nf(IFormFile file = null, int id_usuario_login = 0)
        {
            int linha = 1;
            ...
            try
            {
                ...
                var conteudo = reader.ReadLine();
                HeaderRetornoNF headerretornonf = new HeaderRetornoNF(conteudo);
                ...
                List<DetalheRetornoNF> notas_fiscais = new List<DetalheRetornoNF>();
                while (!reader.EndOfStream)
                {
                    linha++;
                    conteudo = reader.ReadLine();
                    ...
                    {
                        DetalheRetornoNF detalheretornonf = new DetalheRetornoNF(conteudo);
                ...
                {
                      retorno.Add(Registrar_nf(nota_fiscal, id_usuario_login));
                      linha++;
                }
        ...
        private string Baixar_boleto(TransacaoRetornoBoleto boleto, int id_usuario_login = 0)
        {

            string mensagem = "";
            try
            {
                if (Verifica_fatura_cadastrada(id_titulo_banco: boleto.Id_titulo_banco))
                {
                    mensagem = Baixar_fatura(boleto, id_usuario_login);
                }
                else
                {
        ...
        private string Registrar_nf(DetalheRetornoNF nf, int id_usuario_login)
        {

            string mensagem = "";
            try
            {
                if (Verifica_fatura_cadastrada(id: int.Parse(nf.Num_rps)))
                {
                    mensagem = Registrar_nf_fatura(nf, id_usuario_login);
                }
                else
                {
                    mensagem = "Fatura não localizada";
        ...
        private bool Verifica_fatura_cadastrada(string id_titulo_banco = "", int id = 0)
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(Constantes.string_conexao))
            {
                ...
                try
                {
                    ...
                    if (conn.State == ConnectionState.Open)
                    {
                        ...
                        {
                            query = query + "where f.nosso_numero = :nosso_numero";
                            dyParam.Add("nosso_numero", id_titulo_banco, DbType.String, ParameterDirection.Input);
                        }

                        var num_faturas = SqlMapper.QuerySingle(conn, query, param: dyParam, commandType: CommandType.Text);
        ...
        public string Baixar_fatura(TransacaoRetornoBoleto boleto, int id_usuario_login = 0)
        {
        ...
        public string Registrar_nf_fatura(DetalheRetornoNF nf, int id_usuario_login)
        {
            using (NpgsqlConnection conn = new NpgsqlConnection(Constantes.string_conexao))
            {
        ...
        public object Consultar_indicadores(int id_usuario_login)
        {
            object result = null;
